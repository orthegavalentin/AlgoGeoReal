...
//		drawLabel(g);
	...
	public Vecteur Rotate(double angle)
	{
		double cos = Math.cos(angle);
		double sin = Math.sin(angle);
		
		//move triangle at origin
		
		
		
			double newXto = cos * to.x - sin * to.y;
			double newyto = sin * to.x + cos * to.y;
			double newXfrom = cos * from.x - sin * from.y;
			double newYfrom = sin * from.x + cos * from.y;
			
			return new Vecteur(new PointVisible((int)newXfrom,(int)newYfrom),new PointVisible((int)newXto,(int)newYfrom));
